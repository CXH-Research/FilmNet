    ...
    def __init__(self, img_channel=3, width=32, middle_blk_num=12, enc_blk_nums=[2, 2, 4, 8], dec_blk_nums=[2, 2, 2, 2]):
        ...
        for num in enc_blk_nums:
            self.encoders.append(
                nn.Sequential(
                    *[NSRBlock(chan) for _ in range(num)]
                )
            )
            self.downs.append(
        ...
        self.middle_blks = \
            nn.Sequential(
                *[NSRBlock(chan) for _ in range(middle_blk_num)]
            )
...
if __name__ == "__main__":
    ...
    # using('start . ')
    model = LFNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
